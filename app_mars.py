...
import smtplib
from google.oauth2.service_account import Credentials  # se necessário para gspread (mantido conforme original)
...
st.set_page_config(
    page_title="MARS - Torre de Controle com Markup", page_icon="🐾", layout="wide"
)
...
def buscar_preco_na_tabela(arquivo, codigo_produto):
  ...
  try:
    df_p = pd.read_csv(
        caminho_real, sep=";", encoding="utf-8-sig", on_bad_lines="skip"
    )
    df_p.columns = [c.strip().upper() for c in df_p.columns]
    row = df_p[
        df_p["CÓDIGO"].astype(str).str.strip() == str(codigo_produto).strip()
    ]
    if not row.empty:
        ...
        if nomes_internos:
          ...
          with z.open(nome_interno) as f_zip:
            if nome_interno.endswith(".csv"):
              df_v = pd.read_csv(
                  f_zip, sep=";", encoding="latin1", low_memory=False
              )
            else:
...
PRODUTOS_FOCAIS = {
    "99954": "FILEZITOS AD CARNE 60G",
    "99955": "FILEZITOS AD CHURRASCO 60G",
    "99956": "FILEZITOS AD FRANGO 60G",
    "100051": "FILEZITOS AD CHURRASCO 400G",
    "99798": "BISCROK AD RP BANANA 500G",
    "99799": "BISCROK AD RP MACA 500G",
    "98985": "CHAMP ADULTO CARNE & CEREAL 900G",
    "98980": "CHAMP FILHOTES 900G",
    "98679": "KITEKAT DRY AD MIX DE CARNES 900G",
    "99777": "PED TASTY BITES CARNE 130G",
    "99776": "PED TASTY BITES CARNE 40G",
    "99773": "PED TASTY BITES CARNE 80G",
    "99774": "PED TASTY BITES FRANGO 80G",
    "99775": "PED TASTY BITES LEITE 80G",
    "99830": "SHE CRE AT&SAL+FGO&CAM 4X12G",
    "99742": "SHE CRE ATUM 2X12G",
    "99743": "SHE CRE ATUM 4X12G",
    "99744": "SHE CRE ATUM&AT+CAM 4X12G",
    "99831": "SHE CRE FGO&SAL 2X12G",
    "99745": "SHE CRE FRG&FRG+PE 4X12G",
    "98989": "PED NUTRICAO ESSENCIAL AO LEITE 900G",
    "98982": "PED NUTRICAO ESSENCIAL CARNE 900G",
    "98933": "PEDIGREE AD CARNE&FRANG 2,7KG",
    "98934": "PEDIGREE AD CARNE&FRANG 900G",
    "98914": "PEDIGREE AD CARNE&VEG 2,7KG",
    "98915": "PEDIGREE AD CARNE&VEG 900G",
    "98911": "PEDIGREE AD RP 2,7KG",
    "98912": "PEDIGREE AD RP 900G",
    "98930": "PEDIGREE FIL 2,7KG",
    "98931": "PEDIGREE FIL 900G",
    "98899": "WHI AD CARNE 500G",
    "98898": "WHI AD CARNE 900G",
    "98941": "WHI AD FRANGO 900G",
    "98937": "WHI AD PEIXE 500G",
    "98936": "WHI AD PEIXE 900G",
    "98944": "WHI FILHOTES CARNE 500G",
    "98942": "WHI FILHOTES CARNE 900G",
    "98903": "WHI GATO CAST CARNE 500G",
    "98902": "WHI GATO CAST CARNE 900G",
    "98946": "WHI GATOS CAST PEIXE 900G",
    # --- NOVOS PRODUTOS ADICIONADOS ---
    "100056": "WHI SUPERFOODS DRY AD SALM 2,7KG 2025",
    "100055": "WHI SUPERFOODS DRY AD SALM 900G 2025",
    "100058": "WHI SUPERFOODS DRY CASTR SALM 2,7KG 25",
    "100059": "WHI SUPERFOODS DRY CASTR SALM 900G 25",
    "100061": "WHI SUPERFOODS DRY FIL SALMAO 2,7KG 25",
    "100062": "WHI SUPERFOODS DRY FIL SALMAO 900G 25",
    "100094": "BISCROK AD CHURRASCO 500G",
}
...
def gerar_pdf_mars(promotor, loja, cidade, df_audit, df_faltantes, feedback):
  ...
  style_celula_esq = ParagraphStyle(
      "EstiloCelulaEsq",
      parent=estilos["Normal"],
      fontSize=8.5,
      leading=10,
  ...
  style_obs_titulo = ParagraphStyle(
      "ObsTitulo",
      parent=estilos["Normal"],
      fontSize=11,
      leading=13,
  ...
  style_obs_texto = ParagraphStyle(
      "ObsTexto",
      parent=estilos["Normal"],
      fontSize=10,
      leading=14,
  ...
  elementos.append(
      Paragraph(
          "<b>RELATÓRIO DE OPORTUNIDADES E MARKUP MARS</b>", estilos["Title"]
      )
  )
  elementos.append(
      Paragraph(
          f"<b>LOJA:</b> {loja} | <b>CIDADE:</b> {cidade} |"
          f" <b>PROMOTOR:</b> {promotor}",
          estilos["Normal"],
      )
  )
  elementos.append(
      Paragraph(f"<b>DATA/HORA:</b> {dt_pdf}", estilos["Normal"])
  )
  ...
  if not df_audit.empty:
    elementos.append(
        Paragraph(
            "<b>1. AUDITORIA DE PREÇOS, FALTA E MARKUP</b>",
            estilos["Heading3"],
        )
    )
    data_audit = [[
    ...
    for i, row in enumerate(df_audit.to_dict("records")):
      ...
      p_custo_ult = (
          float(p_pago_val)
          if isinstance(p_pago_val, (int, float))
          else 0.0
      )
      ...
      if is_falta or p_loja == 0.0:
        ...
        markup_praticado = (
            ((p_loja - p_custo_ult) / p_custo_ult) * 100 if p_loja > 0 else 0.0
        )
        markup_recomendado = (
            ((p_rec - p_custo_ult) / p_custo_ult) * 100 if p_rec > 0 else 0.0
        )
        ...
        markup_txt = (
            f"<b><font color='{cor_loja}'>{markup_praticado:.0f}%</font></b>"
        )
      else:
        markup_txt = "<b>N/D</b>"
  ...
  if df_faltantes:
    ...
    elementos.append(
        Paragraph(
            "<b>2. PRODUTO NÃO ENCONTRADO NA LOJA / HISTÓRICO</b>",
            estilos["Heading3"],
        )
    )
  ...
  if feedback and feedback.strip():
    ...
    tabela_obs = Table(
        [[
            Paragraph(
                "<b>🗣️ OBSERVAÇÕES / FEEDBACK DO CLIENTE:</b>",
                style_obs_titulo,
            )
...
# --- INTERFACE PRINCIPAL ---
st.markdown(
    "<h1 style='text-align:center;'>🐾 SISTEMA DE OPORTUNIDADES & MARKUP"
    " MARS</h1>",
    unsafe_allow_html=True,
)
...
if "user_mars" not in st.session_state:
  ...
  if promotor == "PAMELA":
    ...
    f_label, arq_precos = (
        ...
        "PAULISTINHAS_MARS_PRECO_ATUALIZADO.csv",
    )
  ...
  df_f = df_vendas[
      df_vendas["CIDADE_BUSCA"].isin(
          [limpar_texto(c) for c in ROTAS_MARS[promotor]]
      )
  ]

  loja = st.selectbox(
      "🏪 Selecione a Loja:", ["-- Selecione --"] + sorted(df_f["CLIENTE NOME"].unique())
  )
  ...
  if loja != "-- Selecione --":
    ...
    for c, n in PRODUTOS_FOCAIS.items():
      ...
      if not historico_item.empty and "DATA" in historico_item.columns:
        historico_item["DATA_DT"] = pd.to_datetime(
            historico_item["DATA"], errors="coerce"
        )
        historico_item = historico_item.sort_values(
            by="DATA_DT", ascending=False
        )
        ...
        info_ultima_compra = (
            f" (Última:"
            f" {dt_ult.strftime('%d/%m/%Y') if pd.notna(dt_ult) else 'Desconhecida'} -"
            f" {op_ult} - Qtd: {int(qtd_ult) if pd.notna(qtd_ult) else 0})"
        )
      else:
        info_ultima_compra = ""
      ...
      preco_pago_exibicao = (
          round(preco_ultimo_pedido, 2)
          if tem_venda
          else "Não foram encontradas vendas este ano"
      )
      ...
      if c in comp_cli:
        ...
        if not historico_item.empty:
          ...
          status_hist = (
              "PRODUTO NÃO ENCONTRADO NA LOJA (NÃO COMPRADO ESTE ANO)"
          )
    ...
    if dados_audit_view:
      ...
      for r in df_edit.to_dict("records"):
        ...
        custo_base = (
            float(p_pago_item)
            if isinstance(p_pago_item, (int, float))
            else 0.0
        )
        ...
        if is_falta_tela or p_loj == 0.0:
          ...
          mk_rec = (
              ((p_sug - custo_base) / custo_base) * 100 if p_sug > 0 else 0.0
          )
          mk_loj = (
              ((p_loj - custo_base) / custo_base) * 100 if p_loj > 0 else 0.0
          )
      ...
      if st.button("🚀 ENVIAR RELATÓRIO"):
        ...
        if tem_preco_zerado:
          st.error(
              "🚨 **Atenção:** Há produtos com o **Preço Gôndola zerado (R$"
              " 0,00)** que **não** estão marcados como falta (`FALTA NA"
              " LOJA?`). Por favor, preencha o preço correto ou marque a caixa"
              " de falta antes de enviar."
          )
        else:
          ...
          for r in df_edit.to_dict("records"):
            status_val = (
                "FALTA"
                if r["FALTA NA LOJA?"] or float(r["PREÇO GÔNDOLA"]) == 0
                else "TEM"
            )
          ...
          pdf_file = gerar_pdf_mars(
              promotor, loja, cidade_cliente, df_edit, prod_faltantes, obs_text
          )

          if enviar_email(
              f"🐾 OPORTUNIDADE & MARKUP: {loja} ({promotor})",
              pdf_file,
              ["benedito.bandola@minassal.com.br"],
          ):
            salvar_nas_planilhas(
                [horario_ref, promotor, loja, cidade_cliente, obs_text],
                detalhado_rows,
            )
            st.success(
                "Relatório gerado e enviado com sucesso para a gestão"
                " (Benedito)!"
            )
            st.balloons()
    else:
      ...
      if st.button("🚨 ENVIAR MIX ZERO"):
        horario_ref = obter_horario_brasil()
        detalhado_rows = [
            [horario_ref, promotor, loja, cidade_cliente, f[0], f[1], f[2], 0.0, 0.0]
            for f in prod_faltantes
        ]
        pdf_file = gerar_pdf_mars(
            promotor,
        ...
        if enviar_email(
          ...
          st.success(
              "Mix Zero registrado e enviado com sucesso para a gestão"
              " (Benedito)!"
          )
          st.balloons()
    ...
    vendas_mars_loja = df_vendas[
        ...
        & (
            df_vendas["FABRICANTE NOME"]
            .astype(str)
            .str.upper()
            .str.contains("MARS", na=False)
        )
    ].copy()
    ...
    if not vendas_mars_loja.empty:
      ...
      soma_qtd = (
          vendas_mars_loja["TOTAL QTD"].sum()
          if "TOTAL QTD" in vendas_mars_loja.columns
          else 0
      )

      st.info(
          f"📊 **Resumo Anual:** {total_compras} registros de pedidos"
          f" encontrados | **Soma Total de Quantidade Comprada:**"
          f" {soma_qtd:,.0f} unidades"
      )

      if "DATA" in vendas_mars_loja.columns:
        vendas_mars_loja["DATA_DT"] = pd.to_datetime(
            vendas_mars_loja["DATA"], errors="coerce"
        )
        vendas_mars_loja["DATA_FORMATADA"] = (
            vendas_mars_loja["DATA_DT"].dt.strftime("%d/%m/%Y")
        )
      else:
        vendas_mars_loja["DATA_DT"] = pd.NaT
        vendas_mars_loja["DATA_FORMATADA"] = ""

      tabela_historico = vendas_mars_loja[[
          "DATA_DT",
          "DATA_FORMATADA",
          "PRODUTO NOME",
          "OPERACAO",
          "TOTAL QTD",
      ]].dropna(subset=["PRODUTO NOME"])
      ...
      st.dataframe(
          tabela_historico.style.map(
              colorir_operacao, subset=["Tipo de Operação"]
          ),
          use_container_width=True,
          hide_index=True,
      )
      ...
      st.info(
          "Nenhum registro de compra de produtos Mars encontrado para esta loja"
          " no período."
      )
